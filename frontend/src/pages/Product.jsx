...
const Product = () => {
  ...
  const {products, formatPKR, addToCart} = useContext(ShopContext);
    ...
    <div className='pt-10 transition-opacity duration-500 ease-in border-t-2 opacity-100'>
      <div className='flex flex-col gap-12 sm:gap-12 sm:flex-row'>
        ...
        <div className='flex-1'>
          ...
          <p className='mt-5 text-3xl font-medium'>{formatPKR(productData.price)}</p>
          <p className='mt-5 text-gray-500 md:w-4/5'>{productData.description}</p>
          <button 
            onClick={() => addToCart(productData._id)}
            className='mt-8 bg-emerald-700 px-8 py-3 text-sm text-white active:bg-emerald-800'
          >
            ADD TO CART
          </button>
      ...
      <div className='mt-20'>
        ...
        <div className='flex flex-col gap-4 px-6 py-6 text-sm text-gray-500 border'>
          <p>Elevate your style with our meticulously crafted KAMAYABI quality products. Designed with a perfect balance of elegance and practicality, these KAMAYABI quality products made from premium materials that ensure both durability and comfort.</p>
          <p>Whether you're dressing up for a special occasion or adding a touch of sophistication to your everyday look, the KAMAYABI quality products offer unparalleled versatility. Its timeless design, coupled with a flawless fit, makes it a must-have addition to any wardrobe. Don’t miss out on the chance to own a piece that combines both form and function—experience the difference today.</p>
        </div>
      </div>
